...
def main():
    ...
    parser.add_argument('--use_audio_mlp', type=bool, default=True,
                        help='Use mlp for audio text (default: True')
    ...
    parser.add_argument('--batch_size_text', type=int, default=100,
                        help='batch size text (default: 100)')
    parser.add_argument('--batch_size_audio', type=int, default=64,
                        help='batch size audio (default: 64)')
    ...
    if args.use_quad_classes:
        ...
        """model_text = LSTM1(input_feature_size=text_features_train.shape[-1], hidden_size=args.hidden_size, n_classes=4,
                           n_layers=args.n_layers, device=device)"""
        model_text = MLP(input_feature_size=text_train.shape[-1], hidden_size=args.hidden_size, n_classes=13,
                            n_layers=args.n_layers, device=device)
        if args.use_esd:
            ...
            if not args.use_text_mlp:
                ...
                for epoch in range(args.n_epochs):
                    ...
                    for idx, batch in enumerate(batches):
                        ...
                        # Calculate the unweighted accuracy of train pre_extracted_features
                        train_acc, f1_train, _, _ = report_acc_mlp(output_text, target_train)
                    ...
                    model_text.eval()
                    with torch.no_grad():
                        output_val_text = model_text(torch.Tensor(text_val).to(device))
                        acc_val_text, f1_val_text, conf_matrix_text, classify_report_text = report_acc_mlp(
                            output_val_text, text_target_val)
                        print(f"Accuracy (evaluation): {acc_val_text} (text_lstm)")
                        ...
                        if best_acc_text > args.save_model_threshold_text:
                            if args.use_quad_classes:
                                torch.save({
                                    ...
                                    "saved_models/text_lstm/mlp/" + "4_model_mlp_acc_" + "{:0.2f}".format(best_acc_text) + ".t")
                            else:
                                torch.save({
                                    'best_epoch': best_epoch_text,
                                    'model_state_dict': model_text.state_dict(),
                                    ...
                                    "saved_models/text_lstm/mlp/" + "3_model_mlp_acc_" + "{:0.2f}".format(best_acc_text) + ".t")
                ...
                print("Text LSTM:")
                print('Best Epoch: {}/{}.............'.format(best_epoch_text, args.n_epochs), end=" ")
                print("Train accuracy: {:.2f}% Evaluation accuracy: {:.2f}% Test accuracy: {:.2f}%".format(train_acc, best_acc_text, acc_test_text))
                print(classify_report_text)
