...
import torch
import numpy as np
from mman.utils import process_features
from utils import process_ESD_features, report_acc_mlp, report_acc
...
_, _, _, _, _, _, data_train, train_label, train_mask, audio_test, audio_test_label, test_mask, _, _, seqlen_train, seqlen_test, _, _ = process_features(True)
...
input_test_mask = torch.Tensor(test_mask)
...
def objective(trial):
    ...
    #model = hir_fullModel(batch_size=10, mode=True, classifier="lstm", output_size=4, hidden_dim=300, fc_dim=200,
    ...
    with torch.no_grad():
        ...
        output_baseline, _ = model(input_data_audio)
        ...
        acc_test, f1_test, conf_matrix, classify_report = report_acc(output_baseline, torch.Tensor(audio_test_label).to(device), input_test_mask)
        if acc_test > 51.9:
