...
def main():
    ...
    parser.add_argument('--full_dict', type=str, default="state_dict/full/full73.94.pt",
                        help='full model pretrained stat dict (default: 73.94)')
    parser.add_argument('--bl_dict', type=str, default="state_dict/bl_hira/bl_hira71.78.pt",
                        help='baseline pretrained stat dict (default: 71.78)')
    ...
    # Define the model
    model = hir_fullModel(batch_size = args.batch_size, mode = args.bl, classifier = args.classifier, output_size = args.output_size, hidden_dim = args.hidden_dim, fc_dim = args.fc_dim , dropout = args.dropout, n_layers = args.n_layers)
    if args.load_dict or args.test_mode:
        if args.bl:
            path = "bl_hira"
            figure_name = "CTF-UD"
            model.load_state_dict(torch.load(args.bl_dict));
        else :
            path = "full"
            figure_name = "UD"
            model.load_state_dict(torch.load(args.full_dict));
    ...
    if args.test_mode:
        ...
        with torch.no_grad():
            ...
            ax.xaxis.set_ticklabels(['HAPY', 'SAD', 'NEUT', 'ANG'],fontsize = 20.0);
            ax.yaxis.set_ticklabels(['HAPY', 'SAD', 'NEUT', 'ANG'],fontsize = 20.0, rotation = 0);
            # ax.get_yaxis().set_visible(False)
            plt.savefig("Figure/"+ figure_name +".pdf", bbox_inches = 'tight', pad_inches = 0)
    else: 
