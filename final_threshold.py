...
threshold_angry = 0.8988932763211875
threshold_happy = 0.12682599408873035
threshold_sad = 0.3095445358242875
threshold_neutral = 0.16726750467583817

model.eval()
esd_model.eval()
with torch.no_grad():
    output_baseline, _ = model(torch.Tensor(np.array(audio_features_test)).to(device))
...
acc_test, f1_test, conf_matrix, classify_report = report_acc(output_baseline, torch.Tensor(audio_labels_test).to(device), audio_mask_test)
